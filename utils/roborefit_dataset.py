...
class RoboRefitDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        exclude_val=False,
        reason_seg_data="RoboRefit|train",
        explanatory=0.1,
    ):
        ...
        jsons = json.load(open(os.path.join(self.base_image_dir, "RoboRefit", split, "roborefit_" + split + '.json'), 'r', encoding="utf-8"))
        self.reason_seg_data = jsons

        print("number of roborefit samples: ", len(jsons))
    ...
    def __getitem__(self, idx):
        ...
        data_info = self.reason_seg_data[idx]
        ...
        mask, sents, is_sentence = cv2.imread(mask_path), [data_info['text']], True
        mask = np.sum(mask, -1)
        mask[mask>0] = 1
